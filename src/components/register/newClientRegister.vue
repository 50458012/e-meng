<template>
  <div class="register">
    <div class="header">
      ...
      <div class="selector">
        <p class="putong" @click="toggleChange">
          <span>普通登录</span>
          <i v-if="isShow"></i>
        </p>
        <p class="iPhone"  @click="toggleChange1">
          <span id="mobile">手机动态密码登录</span>
          <i v-if="!isShow"></i>
        </p>
      </div>
    </div>
    <div class="content">
      <form method="post" id="usually">
        <input type="text" name="username" id="username" placeholder="手机号/邮箱/用户名"/>
        <span ></span>
        <input type="password" name="password" id="pictureContent"  placeholder="请输入图片内容"/>
        <span></span>
  ...
  .register
    ...
    .content
      #username
        ...
        background-position (25/$rem) (16/$rem)
